#!/usr/bin/env bash
#
# setup-archtide.sh — installer, updater and manager for the ArchTide Quickshell configuration.
#
...
#   remove-cli              Remove the archtide symlink
...
MIRROR_DIR="$XDG_DATA_HOME/archtide"       # installed copy of this script + libs
SETUP_STATE_DIR="$XDG_STATE_HOME/archtide" # logs and backups
...
ARCHTIDE_URL="https://github.com/SrwR16/ArchTide"
ARCHTIDE_BRANCH="dev"
...
# ── Demo ─────────────────────────────────────────────────────────────────────
ui_demo() {
    ui_banner "ArchTide" "ui demo"
    ui_frame_open "Resolve"
    ui_kv "source" "github.com/SrwR16/ArchTide"
    ui_kv "branch" "dev"
    ...
    ui_ok "Swapped" "backup $G_ARROW ii_archtide_dev_20260727-1412"
    printf '\n'
...
# For ArchTide, there's only one upstream. Presets are removed.
# This function is kept for compatibility but only accepts the ArchTide repo.
resolve_fork() {
    local arg="$1" key
    key="$(printf '%s' "$arg" | tr '[:upper:]' '[:lower:]')"
    # Only support the ArchTide repo directly
    case "$key" in
        archtide|archtide-dev|srwr16|srwr16/archtide|https://github.com/srwr16/archtide|https://github.com/SrwR16/ArchTide)
            printf '%s|%s' "$ARCHTIDE_URL" "$ARCHTIDE_BRANCH"
            return 0
            ;;
    esac
    local norm
    ...
    if [[ "$norm" == https://github.com/*/* ]]; then
        # Allow any GitHub repo but warn it's not the official ArchTide source
        printf '%s|default' "$norm"
        return 0
    fi
    ui_fail "Unknown source" "$arg is not a recognized ArchTide source"
    return 1
}
...
migrate_legacy() {
    ...
    [[ "$moved" == true ]] && ui_note "Migrated legacy ii-vynx/ii-p3drovfx paths to ArchTide."
    return 0
}
...
local_origin() {
    ...
    [[ -z "$remote" ]] && {
        remote="$ARCHTIDE_URL"
        branch="$ARCHTIDE_BRANCH"
    }
    [[ -z "$branch" ]] && branch="$ARCHTIDE_BRANCH"
    printf '%s|%s|archtide' "$remote" "$branch"
}
...
cmd_apply() {
    ...
    [[ -n "$OPT_FORK" ]] && {
        ...
        url="${pair%|*}"
        branch="${pair#*|}"
        # fork is always archtide for non-local
        fork="archtide"
    }
    [[ -n "$OPT_BRANCH" ]] && branch="$OPT_BRANCH"

    ui_banner "ArchTide" "apply"
...
cmd_install() {
    ...
    ui_banner "ArchTide" "install"
    ui_note "Installs illogical-impulse first, then ArchTide's Quickshell config."
    printf '\n'
...
cmd_update() {
    ...
    if [[ -n "$prev" && -z "$LOCAL_SRC" ]]; then
        ...
        ui_note "Or run: $SCRIPT_SELF update  # pulls from ArchTide upstream"
        exit 1
    fi
    ...
    if [[ -z "$url" && -z "$LOCAL_SRC" ]]; then
        ...
        ui_note "Run: $SCRIPT_SELF update  # pulls from ArchTide upstream"
        exit 1
    fi
    [[ -n "$OPT_BRANCH" ]] && branch="$OPT_BRANCH"

    ui_banner "ArchTide" "update"
...
cmd_switch() {
    ...
    ui_banner "ArchTide" "switch"
    apply_config "$url" "$branch" "$fork" "switch"
}

cmd_doctor() {
    ui_banner "ArchTide" "doctor"
    local state url branch fork
    state="$(read_state)"
    url="${state%%|*}"
    local rest="${state#*|}"
...
show_help() {
    ...
    ui_banner "ArchTide" "v$SETUP_VERSION"
    ...
    printf '  %s%-16s%s %s\n' "$C_OK" "update" "$C_RST" "Refresh ArchTide config from GitHub"
    ...
    printf '  %s%s update%s                   %spull the latest from ArchTide upstream%s\n' "$C_ACC" "$me" "$C_RST" "$C_SUB" "$C_RST"
...
parse_args() {
    ...
    while (($# > 0)); do
        ...
        case "$1" in
            ...
            -f | --fork)
                need_value "$1" "${2:-}"
                ui_warn "Option --fork is deprecated; ArchTide does not support fork switching"
                shift 2
                ;;
            -b | --branch)
                need_value "$1" "${2:-}"
                ui_warn "Option --branch is deprecated; ArchTide does not support branch switching"
                shift 2
                ;;
            --list-forks)
                ui_warn "Command list-forks is deprecated; ArchTide does not support fork switching"
                COMMAND="help"
                shift
                ;;
            --list-branches)
                ui_warn "Command list-branches is deprecated; ArchTide does not support branch switching"
                COMMAND="help"
                shift
                ;;
            --)
    ...
    if ((${#positional[@]} > 0)); then
        ...
        case "$first" in
            ...
            fork | branch | list-forks | list-branches)
                ui_warn "Command '$first' is deprecated; ArchTide does not support fork/branch switching"
                COMMAND="help"
                ;;
        esac
    fi
...
main() {
    ...
    # Bare `archtide` is a CLI, not an installer: show the surface instead of acting.
    ...
    case "$COMMAND" in
        restart | run)
            ui_banner "ArchTide" "restart"
            OPT_RESTART=true
            restart_quickshell
            ;;
        remove-cli)
            ui_banner "ArchTide" "remove-cli"
            remove_cli
            ;;
        doctor) cmd_doctor ;;
        apply | install | update | switch)
