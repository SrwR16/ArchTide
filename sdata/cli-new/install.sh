...
if [[ -n "$EXISTING_SOURCE" && "$CURRENT_DIR" != "$(realpath "$EXISTING_SOURCE" 2>/dev/null)" ]]; then
    ...
    DEFAULT_PATH="$HOME/.local/share/flow-cli"
...
info "Installing Flow CLI to system..."
...
[[ -d "$HOME/.config/fish/completions" ]] && ln -sf "$COMP_SRC/flow.fish" "$HOME/.config/fish/completions/flow.fish"
[[ -d "$HOME/.zsh/completions" ]] && ln -sf "$COMP_SRC/_flow" "$HOME/.zsh/completions/_flow"

success "Installation complete! Use 'flow help' to start."
