...
# Styling commands for Flow Shell
...
cmd_wallpaper() {
    ...
    case "$action" in
        lock|lockscreen)
            info "Opening lockscreen wallpaper selection..."
            $bin -c flow ipc call wallpaper openLock
            success "Lockscreen wallpaper panel opened."
            ;;
        home|desktop|"")
            info "Opening desktop wallpaper selection..."
            $bin -c flow ipc call wallpaper openDesktop
            success "Desktop wallpaper panel opened."
            ;;
        *)
            echo "Usage: flow wallpaper [desktop|lock]"
            ;;
    esac
}
...
cmd_wallpaper_cycle() {
    local action="$1"
    local config_file="$HOME/.config/flow/config.json"
