...
QUICKSHELL_CONFIG_NAME="archtide"
...
SHELL_CONFIG_FILE="$XDG_CONFIG_HOME/archtide/config.json"
...
imgpath="$HOME/.config/quickshell/archtide/assets/images/default_wallpaper.png"
    ...
    if [[ -n "$theme_file" ]]; then
        ...
        if [[ "$type_flag" == "scheme-intense" ]]; then
            echo "[switchwall_archtide.sh] Applying intense surface boost to colors.json (mode: $mode_flag)" >&2
...
main() {
    ...
    if [[ $valid_type -eq 0 ]]; then
        ...
        if [[ -f "$XDG_CONFIG_HOME/archtide/themes/$type_flag.json" ]]; then
            theme_file="$XDG_CONFIG_HOME/archtide/themes/$type_flag.json"
            valid_type=1
    ...
    if [[ $valid_type -eq 0 ]]; then
        echo "[switchwall_archtide.sh] Warning: Invalid type '$type_flag', defaulting to 'auto'" >&2
        type_flag="auto"
    fi
    ...
    if [[ -n "$lockscreen_flag" && -n "$noswitch_flag" ]]; then
        set_wallpaper_path "$imgpath" "lockscreen"
        echo "[switchwall_archtide.sh] Saved lockscreen wallpaper path."
        exit 0
    fi
    ...
    if [[ -n "$lightmode_flag" && -n "$noswitch_flag" ]]; then
        set_wallpaper_path "$imgpath" "lightmode"
        echo "[switchwall_archtide.sh] Saved light mode wallpaper path."
        exit 0
    fi
    ...
    if [[ -n "$theme_file" && "$mode_flag" == "light" ]]; then
        ...
        if [[ -f "$light_theme_file" ]]; then
            ...
            echo "[switchwall_archtide.sh] Using light theme variant: $type_flag"
        else
            echo "[switchwall_archtide.sh] No light variant for '$type_flag', falling back to matugen generation"
            theme_file=""
