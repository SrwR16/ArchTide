...
PRESETS_DIR="$HOME/.config/archtide/presets"
...
case $action in
    ...
    load)
        ...
        if [[ -f "$PRESETS_DIR/$name.json" ]]; then
            ...
            color_engine=$(jq -r '.appearance.colorEngine // "archtide"' "$CONFIG_FILE" 2>/dev/null)
            switch_script="switchwall_archtide.sh"
            if [[ "$color_engine" == "fork" ]]; then
                switch_script="switchwall.sh"
            elif [[ "$color_engine" == "archtide" ]]; then
                switch_script="switchwall_archtide.sh"
            fi
