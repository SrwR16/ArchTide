...
CONFIG_FILE="$HOME/.config/archtide/config.json"
...
if [[ "$REC_SERVICE" == "obs" ]]; then
    if pgrep -x "obs" > /dev/null || pgrep -f "com.obsproject.Studio" > /dev/null; then
        STATUS=$(python3 "$(dirname "$0")/obs_control.py" status 2>/dev/null)
...
if [[ $IS_OBS_RECORDING -eq 1 ]]; then
    ...
    python3 "$(dirname "$0")/obs_control.py" stop
    sleep 1.5
...
if [[ -n "$OBS_CMD" ]]; then
    ...
    python3 "$(dirname "$0")/obs_control.py" start
    ...
    if [[ -n "$LATEST_FILE" ]]; then
        notify-send "Recording Finished" "Saved to: $PWD/$LATEST_FILE" -a 'Recorder' &
        qs -c archtide ipc call launchVideoEditor handle "$PWD/$LATEST_FILE"
    fi
    ...
    if [[ -f "$FILENAME" ]]; then
        ...
        qs -c archtide ipc call launchVideoEditor handle "$PWD/$FILENAME"
    fi
    updatestate false
fi
