...
ContentPage {
    ...
    // ── Active-source state (read from state files in ~/.config/quickshell/ii/) ──
    property string activeRemote: ""
    property string activeBranch: "dev"
    property string activeSource: "archtide"
    ...
    readonly property string setupScript: FileUtils.trimFileProtocol(Directories.home + "/.local/share/archtide/setup-archtide.sh")
    ...
    readonly property string actionLogPath: FileUtils.trimFileProtocol(Directories.home + "/.local/state/archtide/ui-action.log")
    ...
    Process {
        id: stateReadProc
        command: ["bash", "-c",
            "dir=\"$HOME/.config/quickshell/archtide\"; " +
            "out=\"\"; " +
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                page.activeSource   = (parts[2] || "archtide").trim() || "archtide";
    ...
    // ── Action process (run setup-archtide.sh, log into the UI) ──
    Process {
        id: actionProc
    ...
    function runAction(modeName, args) {
        ...
        const cmd = ["bash", "-c", 'mkdir -p "${1%/*}"; : > "$1"; shift; exec "$@"', "archtide-action", page.actionLogPath, "systemd-run", "--user", "--collect", "--wait", "--quiet", "--unit=archtide-action-" + Date.now(), "--property=KillMode=process", "--property=StandardOutput=file:" + page.actionLogPath, "--property=StandardError=inherit",
    ...
    ContentSection {
        ...
        GridLayout {
            ...
            ContentSubsection {
                ...
                bottomRightRadius: Appearance.rounding.verysmall
                title: Translation.tr("ArchTide Info")
                icon: "waves"
    ...
    ContentSection {
        ...
        ContentSubsection {
            ...
            icon: "update"
            tooltip: Translation.tr("Pull latest changes for ArchTide from GitHub and replace the archtide folder")
                ...
                RippleButton {
                    ...
                    contentItem: StyledText {
                        text: {
                            ...
                            return Translation.tr("Update ArchTide @ ") + page.activeBranch;
                        }
                        font.pixelSize: Appearance.font.pixelSize.normal
                        font.weight: Font.Bold
