...
Item {
    ...
    ContentSection {
        ...
        ContentSubsection {
            ...
            tooltip: Translation.tr("Flow: uses the original switchwall pipeline.\n\nFork: uses the fork's color generation pipeline, use this if Flow doesn't work.")
            Layout.fillWidth: true

            ConfigSelectionArray {
                currentValue: Config.options.appearance.colorEngine ?? "flow"
                onSelected: newValue => {
                    Config.options.appearance.colorEngine = newValue;
                }
                options: [
                    {
                        displayName: Translation.tr("Flow"),
                        value: "flow",
                        icon: "verified"
                    },
                    {
