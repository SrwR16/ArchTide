...
ContentPage {
    ...
    NoticeBox {
        Layout.fillWidth: true
        Layout.topMargin: -20
        text: Translation.tr('Not all options are available in this app. You should also check the config file by hitting the "Config file" button on the topleft corner or opening ~/.config/flow/config.json manually.')
        ...
        RippleButtonWithIcon {
            ...
            onClicked: {
                ...
                Quickshell.clipboardText = FileUtils.trimFileProtocol(`${Directories.config}/flow/config.json`);
    ...
    FloatingActionButton {
        ...
        onClicked: {
            Quickshell.execDetached(["bash", "-c", "qs kill -c flow && qs -c flow &"]);
        }
