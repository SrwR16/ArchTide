...
Singleton {
    ...
    readonly property string cacheDir: Directories.home.replace("file://", "") + "/.cache/flow/mpvpaper_thumbs"
    readonly property string ipcSocket: "/tmp/mpvpaper-flow.sock"
    readonly property string framePath: "/tmp/flow_mpvpaper.png"
...
print(json.dumps(out))
    ...
    function stopInternal() {
        ...
        Quickshell.execDetached(["sh", "-c", "pkill -CONT -f 'mpvpaper-flow'; pkill -KILL -f 'mpvpaper-flow'"]);
        root.isPaused = false;
    }

    function pause() {
        if (root.isRunning && !root.isPaused && !root.isApplying) {
            Quickshell.execDetached(["bash", "-c", `echo "set pause yes" | socat - ${root.ipcSocket} 2>/dev/null || pkill -STOP -f 'mpvpaper-flow'`]);
            root.isPaused = true;
        }
    }
    ...
    function resume() {
        if (root.isRunning && root.isPaused) {
            Quickshell.execDetached(["bash", "-c", `echo "set pause no" | socat - ${root.ipcSocket} 2>/dev/null || pkill -CONT -f 'mpvpaper-flow'`]);
            root.isPaused = false;
        }
    }
    ...
    function updatePauseState() {
        ...
        const shellClasses = ["Quickshell", "flow-settings", "flow-monitor", "wayland-dashboard", "waybar", "ags", "fuzzel", "linux-wallpaperengine", "mpvpaper", "mpv"];
