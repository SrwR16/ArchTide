...
PanelWindow {
    ...
    WlrLayershell.namespace: "flow:dock-context-menu"
        ...
        ColumnLayout {
            ...
            ColumnLayout {
                ...
                MenuItem {
                    ...
                    onClicked: { Quickshell.execDetached([Directories.home.replace("file://", "") + "/.config/quickshell/flow/scripts/restartshell.sh"]); root.close() }
                }
                MenuItem {
                    menuText: I18nService.tr("Restart Fix"); menuIcon: "build"
                    onClicked: { Quickshell.execDetached([Directories.home.replace("file://", "") + "/.config/quickshell/flow/scripts/restart_fix.sh"]); root.close() }
                }
                MenuItem {
