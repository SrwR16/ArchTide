...
# System health checks for Flow Shell

cmd_doctor() {
    info "Running Flow Health Check..."
    ...
    local venv_path="$HOME/.local/share/flow/venv"
    if [[ -d "$venv_path" ]]; then
    ...
    local state_file="$HOME/.config/flow/install_state.json"
    ...
    success "Health check complete. Use 'flow install deps' to fix missing dependencies."
}
