...
# Maintenance commands for Flow Shell

SHELL_STATE_FILE="$HOME/.config/flow/install_state.json"
CLI_STATE_FILE="$HOME/.config/flow/cli_state.json"
...
cmd_update() {
    ...
    if [[ "$target" == "shell" || -z "$target" ]]; then
        ...
        if [[ -z "$install_dir" ]]; then
            error "Flow Shell installation directory not found. Please run 'flow install' first."
        fi
        ...
        echo "Usage: flow update {shell|cli} [stable|canary]"
    fi
}

cmd_install_shell() {
    info "Installing/Setting up Flow Shell..."
    if command -v curl &> /dev/null; then
        bash -c "$(curl -fsSL https://raw.githubusercontent.com/na-ive/flow-shell/main/install.sh)"
    else
